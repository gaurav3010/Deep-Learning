...
""" -------------------------- Multy Class Classification of FFNetwork --------------------------"""
...
X_train, X_val, Y_train, Y_val = train_test_split(data, labels, stratify = labels_orig, random_state = 0)
print(X_train.shape, X_val.shape)
print(Y_train.shape)
...
ffsnn_multi = FFSNMultiClassNetwork(2, 4, [2, 3])
ffsnn_multi.fit(X_train, y_OH_train, epochs=2000, learning_rate = 0.005, display_loss = True)


# Train predict for see the accuracy

Y_pred_train = ffsnn_multi.predict(X_train)
...
Y_pred_val = ffsnn_multi.predict(X_val)
...
plt.scatter(X_train[:, 0], X_train[:, 1], c = Y_pred_train, s = 15*(np.abs(Y_pred_train - Y_train) + 0.2))
